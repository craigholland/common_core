...
- 'delete_child(str)': Deletes the dataclass instance. (All children are also deleted)  # noqa
- 'reset(cascade: bool)': Resets the dataclass instance to its default values.
                            cascade=False (default)Children are NOT reset
                            cascade=True Children are reset to their default values.    # noqa
...
Ex:
# `fields` is a dictionary of the fields to be created with their default values.   # noqa
...
print(main.asdict)  # {'metafield1': True, 'metafield2': False, 'metafield3': False}    # noqa
...
child = main.get_child('child2', 'grandchild1') # Returns the grandchild1 instance  # noqa
child = main.get_child('child5') # Raises an exception
child = main.get_child('child5', auto_create=True) # Creates a new child5 instance  # noqa
...
class NestedDataclassError:
    ...
    FIELD_TYPE_MISMATCH = (
        ERR_PFX + "Field `{0}` must be of type {1}. Got {2} instead."
    )
    MISSING_CHILD = (
        ERR_PFX + "get_child() - At least one child name must be " "provided."
    )
...
def nested_dataclass_factory(name: str, field_dct: dict, parent=None):
    ...
    for field_name, field_value in field_dct.items():
        fields.append(
            (field_name, type(field_value), field(default=field_value))
        )
    return make_dataclass(name, fields=fields, bases=(BaseNestedDataclass,))(
        _parent=parent
    )
...
class BaseNestedDataclass:
    ...
    def add_child(self, child_name: str, force=False):
        if child_name in self._children and not force:
            raise KeyError(
                NestedDataclassError.DUPLICATE_CHILD.format(child_name)
            )
    ...
    def get_child(self, *child_names, auto_create=False):
        ...
        if child_name not in self._children and auto_create:
            ...
            raise KeyError(
                NestedDataclassError.CHILD_NOT_FOUND.format(child_name)
            )
    ...
    def __setattr__(self, key, value):
        if getattr(self, "_initialized", False):
            if key not in [f.name for f in self._fields]:
                raise KeyError(
                    NestedDataclassError.FIELD_NOT_FOUND.format(key)
                )
            if not isinstance(value, type(getattr(self, key))):
                raise TypeError(
                    NestedDataclassError.FIELD_TYPE_MISMATCH.format(
