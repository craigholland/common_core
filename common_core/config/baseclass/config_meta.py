...
from common_core.utils.baseclass import YAMLLoader, nested_dataclass_factory, BaseNestedDataclass
...
# _LOCK_ATTRS_ON_INIT_VAR is the name of the class attribute that ConfigMeta will
# look for in determining if to lock the class attributes once the class is
# created.
# A Locked class cannot add new attributes. (Default: True)
_LOCK_ATTRS_ON_INIT_VAR = "_LOCK_ATTRS_ON_INIT"

# _LOCK_VALUE_ON_INIT_VAR is the name of the class attribute that ConfigMeta will
# look for in determining if to lock the values after initialization.
# A Locked class change attribute Values. (Default: True)
...
class ConfigMeta(type):
    ...
    def __new__(mcs, name, bases, attrs, *args, **kwargs):
        ...
        new_class._local_meta: BaseNestedDataclass = (
            mcs._build_metadata(new_class))
    ...
    def _build_metadata(mcs, klass):
        ...
        if len(mro_keys) == 1:
            ...
            klass._metadata = nested_dataclass_factory(mro_keys[0], default_fields)
            klass._metadata.top_parent = True
            return klass._metadata
        else:
            child_meta = klass._metadata.get_child(*mro_keys[1:], auto_create=True)
    ...
    def set_attr(mcs, cls):
        def _set_attr(cls, key, value):
            if not cls._local_meta.lock_values or not cls._local_meta.initialized:
                super().__setattr__(key, value)
            else:
    ...
    @staticmethod
    def __generate_config_field(name: str, metadata: dict = None, by_value=None):
    ...
    def __class_fields(mcs, klass):
        ...
        # Get Class
        for attr in [a for a in vars(klass) if a != _YAML_FILE_VAR and is_valid_envvar_name(a)]:
        ...
        for field in class_fields:
            ...
            for priority in reversed(ConfigEnvVarType_Priority):
                ...
                if (priority == ConfigEnvVarType.OS_ENVIRON and
                    ...
                    new_value = ConfigValue(field, new_value, priority, klass.__name__)
                elif (priority == ConfigEnvVarType.CONFIG_CLASS and
                      field.name in vars(klass)):
                    ...
                    if not isinstance(yaml_value, (dict, type(None))):
                        ...
                        new_value = ConfigValue(field, yaml_value, priority, klass.__name__)
                else:
                    continue
                if new_value:
                    ...
                    new_value = ConfigValue(
                        field, field.default,
                        ConfigEnvVarType.CONFIG_CLASS, klass.__name__)
                    value = value.compare(new_value) if value else new_value
            if value:
                values.append(value)
        return values
