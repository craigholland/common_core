...
class ConfigFieldError:
    ...
    INVALID_KEY = (
            ERR_PFX + "ConfigField has no metadata field '{0}'."
    )
...
class ConfigField:
    ...
    datatype: [type, List[type]] = type(None)
    alt_name: str = ''
    required: bool = False
    default: Any = None
    locked: bool = False
    ...
    def _set_default_datatype(self):
        if self.datatype is type(None) and self.default is not None:
            self.datatype = type(self.default)
        elif self.datatype is type(None):
        ...
        if datatype is Any:
            ...
            if list_of_types := [x for x in datatype if isinstance(x, _GenericAlias)]:
                for genAlias in list_of_types:
                    typeset = tuple([t for t in genAlias.__args__ if isinstance(t, type)])
                    if isinstance(value, list):
                        valid = valid or all([isinstance(v, typeset) for v in value])
                        if valid:
                            break
            return valid
    ...
    def _validate_datatype(cls, value: [str, type, list]):
        ...
        if isinstance(value, str):
            ...
            dtypes = []
            for dtype in value:
                if cls._validate_datatype(dtype) and not isinstance(dtype, (list, tuple)):
                    dtypes.append(dtype)
                else:
                    return None
    ...
    def __validate_name(self, name=None, field='name'):
        ...
        if illegal_char := set(name).difference(VALID_VARNAME_CHARS):
            raise ValueError(
                ConfigFieldError.NAME_ILLEGALCHAR.format(field, ', '.join(list(illegal_char)), name)
            )
